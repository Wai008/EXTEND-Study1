...
```{r}
# Median and IQR for each age group - total
totalmedian_iqr_by_privpracloc <- SCDKAPa1 %>%
...
totalmedian_iqr_by_privpracloc

# Confidence Intervals of Total % correct for each practice location
ci_by_privpracloc <- SCDKAPa1 %>%
  group_by(practice_location_ng___4) %>%
  summarize(
    ci_lower = t.test(total_percentcorrect)$conf.int[1],
    ci_upper = t.test(total_percentcorrect)$conf.int[2]
...
```{r}
# Median and IQR for each age group - complications
compmedian_iqr_by_privpracloc <- SCDKAPa1 %>%
  group_by(practice_location_ng___4) %>%
  summarize(
    median_correct = median(comp_percentcorrect, na.rm = TRUE),
    q1 = quantile(comp_percentcorrect, probs = 0.25, na.rm = TRUE),
...
compmedian_iqr_by_privpracloc
```

Management
```{r}
# Median and IQR for each age group - management
managemedian_iqr_by_privpracloc <- SCDKAPa1 %>%
  group_by(practice_location_ng___4) %>%
  summarize(
    median_correct = median(manage_percentcorrect, na.rm = TRUE),
    q1 = quantile(manage_percentcorrect, probs = 0.25, na.rm = TRUE),
...
managemedian_iqr_by_privpracloc
```

Biopsychosocial
```{r}
# Median and IQR for each age group - bps
bpsmedian_iqr_by_privpracloc <- SCDKAPa1 %>%
  group_by(practice_location_ng___4) %>%
  summarize(
    median_correct = median(bps_percentcorrect, na.rm = TRUE),
    q1 = quantile(bps_percentcorrect, probs = 0.25, na.rm = TRUE),
...
```{r}
# Median and IQR for each age group - total
totalmedian_iqr_by_othpracloc <- SCDKAPa1 %>%
...
totalmedian_iqr_by_othpracloc

# Confidence Intervals of Total % correct for each practice location
ci_by_othpracloc <- SCDKAPa1 %>%
  group_by(practice_location_ng___5) %>%
  summarize(
    ci_lower = t.test(total_percentcorrect)$conf.int[1],
    ci_upper = t.test(total_percentcorrect)$conf.int[2]
...
ci_by_othpracloc # Negative lower CI and really high upper CI, probably due to small sample size?
```

Complications
```{r}
# Median and IQR for each age group - complications
compmedian_iqr_by_othpracloc <- SCDKAPa1 %>%
  group_by(practice_location_ng___5) %>%
  summarize(
    median_correct = median(comp_percentcorrect, na.rm = TRUE),
    q1 = quantile(comp_percentcorrect, probs = 0.25, na.rm = TRUE),
...
compmedian_iqr_by_othpracloc
```

Management
```{r}
# Median and IQR for each age group - management
managemedian_iqr_by_othpracloc <- SCDKAPa1 %>%
  group_by(practice_location_ng___5) %>%
  summarize(
    median_correct = median(manage_percentcorrect, na.rm = TRUE),
    q1 = quantile(manage_percentcorrect, probs = 0.25, na.rm = TRUE),
...
managemedian_iqr_by_othpracloc
```

Biopsychosocial
```{r}
# Median and IQR for each age group - bps
bpsmedian_iqr_by_othpracloc <- SCDKAPa1 %>%
  group_by(practice_location_ng___5) %>%
  summarize(
    median_correct = median(bps_percentcorrect, na.rm = TRUE),
    q1 = quantile(bps_percentcorrect, probs = 0.25, na.rm = TRUE),
...
### PRACTICE LOCATION - WILCOXONRANKSUM ###
