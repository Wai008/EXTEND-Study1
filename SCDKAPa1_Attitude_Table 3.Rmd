...
Based on the results of histogram, shapiro test, kurtosis and skewness tests, it is confirmed that data are **not** normally distributed.
...
## Calculating median/IQR for attitude variables grouping by profession. **Please be careful** when checking the prof data. 1 (Physician), 2 (Nurse), 3 (Other).
...
## Calculating median/IQR for attitude variables grouping by Patient population. **1 (adult), 2 (paed), 3 (both)**
...
* group 2 to 5 will be collapsed as a group and will be compared with group 1.

```{r}
ptseen_number <- SCDKAPa1 %>% # Creating new variable to collapse group 2 to 5
  mutate(num_pt = ifelse(numscdpt_ng == 1, 1, 2))
ptseen_number

ptseen_pos <- ptseen_number %>% # Positive attitude
  group_by(num_pt) %>% 
  summarise(median(sum_positive_l, na.rm = TRUE),
            quantile_25_neg = quantile(sum_positive_l, 0.25, na.rm = TRUE),
            quantile_75_neg = quantile(sum_positive_l, 0.75, na.rm = TRUE))
...
ptseen_neg <- ptseen_number %>% # Negative attitude
  group_by(num_pt) %>% 
  summarise(median(sum_negative_l, na.rm = TRUE),
            quantile_25_neg = quantile(sum_negative_l, 0.25, na.rm = TRUE),
            quantile_75_neg = quantile(sum_negative_l, 0.75, na.rm = TRUE))
...
ptseen_con <- ptseen_number %>% # Concerns attitude
  group_by(num_pt) %>% 
  summarise(median(sum_concerns_l, na.rm = TRUE),
            quantile_25_neg = quantile(sum_concerns_l, 0.25, na.rm = TRUE),
            quantile_75_neg = quantile(sum_concerns_l, 0.75, na.rm = TRUE))
...
ptseen_red <- ptseen_number %>% # Redflag attitude
  group_by(num_pt) %>% 
  summarise(median(sum_redflag_l, na.rm = TRUE),
            quantile_25_neg = quantile(sum_redflag_l, 0.25, na.rm = TRUE),
            quantile_75_neg = quantile(sum_redflag_l, 0.75, na.rm = TRUE))
...
# Kruskal Wallis test by using total sum of all variables attirbuting to one specific attitude if we decide data is **not** normally distributed.
...
```{r}
kruskal.test(sum_positive_l ~ num_pt, data = ptseen_number)

kruskal.test(sum_negative_l ~ num_pt, data = ptseen_number)

kruskal.test(sum_concerns_l ~ num_pt, data = ptseen_number)

kruskal.test(sum_redflag_l ~ num_pt, data = ptseen_number)
```
...
sum_pos_ci <- SCDKAPa1 %>%
  do(tidy(wilcox.test(sum_positive_l ~ 1, data = ., conf.int = TRUE, exact = FALSE, correct = FALSE))) %>%
  mutate(
    method = "wilcoxon",
...
```{r}
sum_neg_ci <- SCDKAPa1 %>%
  do(tidy(wilcox.test(sum_negative_l ~ 1, data = ., conf.int = TRUE, exact = FALSE, correct = FALSE))) %>%
  mutate(
    method = "wilcoxon",
    estimate_type = "Hodges–Lehmann"
  )
...
```{r}
sum_con_ci <- SCDKAPa1 %>%
  do(tidy(wilcox.test(sum_concerns_l ~ 1, data = ., conf.int = TRUE, exact = FALSE, correct = FALSE))) %>%
  mutate(
    method = "wilcoxon",
    estimate_type = "Hodges–Lehmann"
  )
...
```{r}
sum_red_ci <- SCDKAPa1 %>%
  do(tidy(wilcox.test(sum_redflag_l ~ 1, data = ., conf.int = TRUE, exact = FALSE, correct = FALSE))) %>%
  mutate(
    method = "wilcoxon",
    estimate_type = "Hodges–Lehmann"
  )
